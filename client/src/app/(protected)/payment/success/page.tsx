"use client";

import { useEffect } from "react";
import Link from "next/link";
import { CheckCircle2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useAppDispatch } from "@/store/hooks";
import { rehydrateAuth } from "@/store/features/auth/authThunks";

export default function PaymentSuccessPage() {
  const dispatch = useAppDispatch();

  useEffect(() => {
    dispatch(rehydrateAuth());
  }, [dispatch]);

  return (
    <main className="min-h-screen bg-black px-6 pb-16 pt-28 text-white md:px-8">
      <div className="mx-auto max-w-md text-center">
        <CheckCircle2
          className="mx-auto size-14 text-green"
          strokeWidth={1.5}
          aria-hidden
        />
        <h1 className="mt-6 text-2xl font-semibold tracking-tight">
          Payment successful
        </h1>
        <p className="mt-3 text-sm text-white/60">
          Credits are added after Stripe confirms payment (usually within a few
          seconds). We refreshed your account — if totals look old, open Profile
          again in a moment.
        </p>
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <Button
            asChild
            className="rounded-full bg-green px-6 text-black hover:brightness-95"
          >
            <Link href="/profile">View profile</Link>
          </Button>
          <Button
            asChild
            variant="outline"
            className="rounded-full border-white/20 bg-transparent"
          >
            <Link href="/generate">Build a site</Link>
          </Button>
        </div>
      </div>
    </main>
  );
}
